# Coverset — Full-Fledged Specification Draft

This is a proposed replacement-style specification drafted alongside `SPEC.md` rather than
editing it in place. `PROJECT_BRIEF.md` remains the product narrative; this document is
the implementation contract: what must be true, what data crosses each boundary, how it
is verified, and which parts are in the active demo path.
...
| **Solver** (CP-SAT) | Produces candidate boards | The only thing that may |
...
| CST-009 | Any scene/work item referencing cast validates all cast IDs against the active roster before solving. | unit-built | offline | MVP-0 | Existing coverage path validates; solver work items must also. |
...
| TRK-004 | Search, Extract, and downstream grounded values in one replan share a Parallel session or explicit correlation ID. | unit-built | live | MVP-1 | Existing GRD-008 behavior. |
...
| DAY-008 | Deterministic daylight algorithms are rerun during solve/replan; they are not monitored as mutable external sources. | unit-built | offline | MVP-0 | Spec clarification. |
...
| CON-008 | Daylight constraints cite the deterministic algorithm/version rather than source URLs. | unit-built | offline | MVP-0 | Existing daylight provenance partially covers. |
...
| AUD-002 | Every constraint traces to either source URL/value provenance, a named deterministic algorithm, or a human-entered production rule. | domain-model | offline/live | MVP-1 | Evidence/daylight portions exist; full ConstraintRecord missing. |
...
**Exercises:** CON-001, CON-002, CON-003, CON-005, CON-007, CST-001, CST-003, GRD-001, GRD-002, GRD-003, GRD-005, GRD-012, GRD-013, TRK-001, TRK-002
...
**Exercises:** TRK-003, MON-001, MON-002, MON-003, MON-006, MON-007, GRD-006, GRD-010, WEA-001, WEA-002, WEA-003, CST-004, CST-005, SOL-004, SOL-012, LCK-001, LCK-002, ACT-004, ACT-007, OUT-002, OUT-005
