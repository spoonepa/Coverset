...
export default function Home() {
  ...
  async function runUploadedScreenplay() {
    ...
    try {
      ...
      setStatus('Running Gemini breakdown and auto-accepting schedulable candidates...');
  ...
  return (
    <main className="shell">
      <section className="hero">
        ...
        <h1>Screenplay to stripboard, with agents kept out of the decision path.</h1>
        <p>
          Gemini proposes candidate scenes, humans or dev auto-review accept schedulable records,
          and the deterministic CP-SAT scheduler produces the board.
        </p>
      </section>
      ...
      <section className="panel grid">
        <div>
          <h2>Fast smoke</h2>
          <p>Runs the authored fixture through the deployed API, DB layer, and scheduler.</p>
        ...
        <div>
          ...
          <button type="button" onClick={runUploadedScreenplay}>Upload, break down, solve</button>
        </div>
      </section>
      ...
      {production && (
        <section className="panel meta">
          ...
          {breakdown && <span>Accepted scenes: {acceptedCount}/{breakdown.candidates.length}</span>}
        </section>
      )}
        ...
        <section className="panel">
          ...
          <div className="sceneList">
            {breakdown.candidates.map((candidate) => (
              <article key={candidate.id} className={candidate.accepted ? 'scene accepted' : 'scene'}>
                <div>
                  <strong>{candidate.scene_number}</strong> {candidate.slugline}
                </div>
                <small>
                  {candidate.int_ext}/{candidate.day_night} · {candidate.location_ref} · cast:{' '}
                  {candidate.cast_ids.join(', ') || '-'} · flags: {flagText(candidate.flags)}
                </small>
                {candidate.resolution_errors.length > 0 && (
                  <small className="errorText">{candidate.resolution_errors.join('; ')}</small>
                )}
              </article>
            ))}
          </div>
