...
def upgrade() -> None:
    if "approval_state" not in _columns("boards"):
        op.add_column(
            "boards",
            sa.Column("approval_state", sa.String(40), nullable=True, server_default="approved"),
        )
    ...
    if "replan_requests" in _tables():
        ...
        with op.batch_alter_table("replan_requests") as batch:
            if "source_kind" not in replan_columns:
                batch.add_column(sa.Column("source_kind", sa.String(40), nullable=True, server_default="monitor"))
            if "source_id" not in replan_columns:
                batch.add_column(sa.Column("source_id", sa.String(48), nullable=True, server_default=""))
            if "reason" not in replan_columns:
                batch.add_column(sa.Column("reason", sa.Text(), nullable=True, server_default=""))
            if "finding_id" in replan_columns:
                batch.alter_column("finding_id", existing_type=sa.String(48), nullable=True)
    ...
    if "constraint_proposals" not in _tables():
        ...
        op.create_index("ix_constraint_proposals_production_id", "constraint_proposals", ["production_id"])
    ...
    if "grounded_values" not in _tables():
        op.create_table(
            ...
            sa.Column("id", sa.String(48), primary_key=True),
            sa.Column("production_id", sa.String(48), sa.ForeignKey("productions.id"), nullable=False),
            sa.Column("evidence_id", sa.String(48), sa.ForeignKey("grounding_evidence.id"), nullable=False),
        ...
        op.create_index("ix_grounded_values_production_id", "grounded_values", ["production_id"])
        op.create_index("ix_grounded_values_evidence_id", "grounded_values", ["evidence_id"])
    ...
    if "monitored_sources" not in _tables():
        op.create_table(
            ...
            sa.Column("id", sa.String(48), primary_key=True),
            sa.Column("production_id", sa.String(48), sa.ForeignKey("productions.id"), nullable=False),
            sa.Column("board_id", sa.String(48), sa.ForeignKey("boards.id"), nullable=False),
            sa.Column("source_url", sa.Text(), nullable=False),
            sa.Column("fact_kind", sa.String(40), nullable=False),
        ...
        op.create_index("ix_monitored_sources_production_id", "monitored_sources", ["production_id"])
        op.create_index("ix_monitored_sources_board_id", "monitored_sources", ["board_id"])
    ...
    if "monitor_change_events" not in _tables():
        op.create_table(
            ...
            sa.Column("id", sa.String(48), primary_key=True),
            sa.Column("production_id", sa.String(48), sa.ForeignKey("productions.id"), nullable=False),
            sa.Column("monitored_source_id", sa.String(48), sa.ForeignKey("monitored_sources.id"), nullable=True),
            sa.Column("board_id", sa.String(48), sa.ForeignKey("boards.id"), nullable=False),
            sa.Column("status", sa.String(40), nullable=True),
        ...
        op.create_index("ix_monitor_change_events_production_id", "monitor_change_events", ["production_id"])
        op.create_index("ix_monitor_change_events_monitored_source_id", "monitor_change_events", ["monitored_source_id"])
        op.create_index("ix_monitor_change_events_board_id", "monitor_change_events", ["board_id"])
    ...
    if "schedule_diffs" not in _tables():
        op.create_table(
            ...
            sa.Column("id", sa.String(48), primary_key=True),
            sa.Column("production_id", sa.String(48), sa.ForeignKey("productions.id"), nullable=False),
            sa.Column("base_board_id", sa.String(48), sa.ForeignKey("boards.id"), nullable=False),
            sa.Column("revised_board_id", sa.String(48), sa.ForeignKey("boards.id"), nullable=False),
            sa.Column("replan_request_id", sa.String(48), sa.ForeignKey("replan_requests.id"), nullable=True),
        ...
        op.create_index("ix_schedule_diffs_production_id", "schedule_diffs", ["production_id"])
        op.create_index("ix_schedule_diffs_base_board_id", "schedule_diffs", ["base_board_id"])
        op.create_index("ix_schedule_diffs_revised_board_id", "schedule_diffs", ["revised_board_id"])
        op.create_index("ix_schedule_diffs_replan_request_id", "schedule_diffs", ["replan_request_id"])
    ...
    if "coverage_items" not in _tables():
        op.create_table(
            ...
            sa.UniqueConstraint("production_id", "coverage_key", name="uq_coverage_items_key"),
        )
        op.create_index("ix_coverage_items_production_id", "coverage_items", ["production_id"])
    ...
    if "coverage_findings" not in _tables():
        op.create_table(
            ...
            sa.Column("id", sa.String(48), primary_key=True),
            sa.Column("production_id", sa.String(48), sa.ForeignKey("productions.id"), nullable=False),
            sa.Column("coverage_item_id", sa.String(48), sa.ForeignKey("coverage_items.id"), nullable=False),
            sa.Column("board_id", sa.String(48), sa.ForeignKey("boards.id"), nullable=True),
            sa.Column("status", sa.String(40), nullable=True),
        ...
        op.create_index("ix_coverage_findings_production_id", "coverage_findings", ["production_id"])
        op.create_index("ix_coverage_findings_coverage_item_id", "coverage_findings", ["coverage_item_id"])
    ...
    if "pickup_tasks" not in _tables():
        op.create_table(
            ...
            sa.Column("id", sa.String(48), primary_key=True),
            sa.Column("production_id", sa.String(48), sa.ForeignKey("productions.id"), nullable=False),
            sa.Column("finding_id", sa.String(48), sa.ForeignKey("coverage_findings.id"), nullable=False),
            sa.Column("coverage_item_id", sa.String(48), sa.ForeignKey("coverage_items.id"), nullable=False),
            sa.Column("board_id", sa.String(48), sa.ForeignKey("boards.id"), nullable=True),
            sa.Column("status", sa.String(40), nullable=True),
            sa.Column("scene_id", sa.String(120), nullable=False),
        ...
        op.create_index("ix_pickup_tasks_production_id", "pickup_tasks", ["production_id"])
        op.create_index("ix_pickup_tasks_finding_id", "pickup_tasks", ["finding_id"])
        op.create_index("ix_pickup_tasks_coverage_item_id", "pickup_tasks", ["coverage_item_id"])
...
def downgrade() -> None:
    ...
    if "coverage_findings" in _tables():
        op.drop_index("ix_coverage_findings_coverage_item_id", table_name="coverage_findings")
        op.drop_index("ix_coverage_findings_production_id", table_name="coverage_findings")
    ...
    if "schedule_diffs" in _tables():
        op.drop_index("ix_schedule_diffs_replan_request_id", table_name="schedule_diffs")
    ...
    if "monitor_change_events" in _tables():
        op.drop_index("ix_monitor_change_events_board_id", table_name="monitor_change_events")
        op.drop_index("ix_monitor_change_events_monitored_source_id", table_name="monitor_change_events")
        op.drop_index("ix_monitor_change_events_production_id", table_name="monitor_change_events")
    ...
    if "monitored_sources" in _tables():
        ...
        op.drop_index("ix_monitored_sources_production_id", table_name="monitored_sources")
    ...
    if "constraint_proposals" in _tables():
        op.drop_index("ix_constraint_proposals_production_id", table_name="constraint_proposals")
