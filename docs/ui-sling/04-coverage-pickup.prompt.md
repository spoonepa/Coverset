...
Create the Coverage Review/pickup screen. It must show Gemini Advisory finding, Needs Director Review status, the warning Gemini can flag but only a human can decide, buttons Accept Coverage / Reject Coverage / Request Pickup / Add Human Finding, and a pickup task form with cast, location, duration, priority, must-complete-by.
