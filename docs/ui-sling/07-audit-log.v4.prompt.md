...
Create a dark dense Audit Log screen. Must show timestamped rows: Parallel Search called for weather, Parallel Extract retrieved full content, Gemini Constraint Agent typed candidate value, Policy/Validator activated constraint, CP-SAT Solver generated board proposal, Independent Validator validated board, First AD selected option, Director requested pickup, UPM / Line Producer approved added shoot-day cost. Use supported icons only; no broken icon names like data_extraction. Legend must say daylight is computed locally and CP-SAT generates while independent validator validates.