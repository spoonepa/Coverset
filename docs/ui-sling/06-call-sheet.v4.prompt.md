...
Create a dark dense Call Sheet Preview screen for Second AD. Must include production/date/day, generated from schedule v4 after board validation, scenes list, locations, cast call-time table, crew call, estimated wrap, daylight window, turnaround notes, permit notes, weather, recipients as read-only viewers, and buttons Generate Call Sheet, Preview PDF, Send to Cast & Crew. Use Savannah/Eastern Time sample data.