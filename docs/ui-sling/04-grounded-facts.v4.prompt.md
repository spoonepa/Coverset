...
Create a dark dense Grounded Facts screen. Must show Parallel Search and Parallel Extract provenance, source URL, source span/table row, content hash, provider response ID, retrieval timestamp, normalized value, validator result. Use Savannah Film Office / City of Savannah permit source and Eastern Time. Show weather.gov/NWS for weather. Must not use City of LA, filmala.com, filmla.com, Pasadena, or PST. Must show daylight as computed locally, not retrieved/grounded.