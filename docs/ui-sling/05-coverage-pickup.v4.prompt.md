...
Create a dark dense Coverage Review and Pickup screen. Must show Gemini Advisory finding on Scene SC-018 reverse, Needs Director Review, warning Gemini can flag this; only a human can decide it. Prominent Director decision buttons: Accept Coverage, Reject Coverage, Request Pickup, Add Human Finding. Pickup task spec form: scene, shot type, cast, location, estimated duration, priority, must complete by. Cost warning only says UPM approval required if selected board adds a shoot day.