...
---
Use SPEC.md as product truth. Relevant spec excerpt follows:
...
---
Generate the missing desktop screen: Scene breakdown / review.
This screen supports the Gemini upload flow. It must show candidate SceneRecord rows, low-confidence fields, accept/reject controls, resolve cast/location controls, and convert-to-work-item action. Include a left nav and top context bar consistent with the other screens.
Required content: scenes table with SC-003, SC-007, SC-012, SC-014, SC-018, SC-021; statuses Active, Candidate, Needs Review; confidence badges; unresolved location/cast warnings; right inspector with SceneRecord fields; buttons Accept Scene Record, Reject Candidate, Send to Review, Resolve Cast, Resolve Location, Convert to Work Item.
Authority constraints: Gemini produces candidate scene records only. Human acceptance or validation is required before activation. Unresolved cast/location blocks solving.