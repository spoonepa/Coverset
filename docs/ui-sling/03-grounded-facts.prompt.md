...
Create the Grounded Facts/source panel screen. It must emphasize Parallel Search at runtime, Extracted Full Content, source URL, source span/table row, retrieval timestamp, validator result, normalized Weather Forecast value, and conflict/error states. Avoid vague AI confidence as the only provenance.
